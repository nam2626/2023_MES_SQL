...
--월별 최다 판매 차량 대수를 조회
--1. 차량별 총 월별 판매대수 조회
SELECT TO_CHAR(CS.CAR_SELL_DATE,'MM') AS GROUP_MONTH, 
CS.CAR_NO, SUM(CS.CAR_SELL_EA) AS SUM_SELL_EA
FROM CAR_SELL CS
GROUP BY TO_CHAR(CS.CAR_SELL_DATE,'MM'), CS.CAR_NO;
--2. 1번 데이터를 기준으로 월별 최대값 조회
SELECT GROUP_MONTH, MAX(SUM_SELL_EA) AS MAX_EA
FROM 
(SELECT TO_CHAR(CS.CAR_SELL_DATE,'MM') AS GROUP_MONTH, 
CS.CAR_NO, SUM(CS.CAR_SELL_EA) AS SUM_SELL_EA
FROM CAR_SELL CS
...
--3. 1번 2번을 기준으로 월별 최다 판매 차량 대수를 조회
SELECT GROUP_MONTH, CAR_NO, SUM_SELL_EA
FROM 
(SELECT TO_CHAR(CS.CAR_SELL_DATE,'MM') AS GROUP_MONTH, 
CS.CAR_NO, SUM(CS.CAR_SELL_EA) AS SUM_SELL_EA
FROM CAR_SELL CS
