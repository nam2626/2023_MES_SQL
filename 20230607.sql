...
SELECT 
(SELECT CM.CAR_MAKER_NAME FROM CAR_MAKER CM
WHERE CS.CAR_MAKER_NO = CM.CAR_MAKER_NO) AS CAR_MAKER_NAME,
CS.CAR_AVG_PRICE, CS.CAR_MAKER_COUNT 
FROM
(SELECT C.CAR_MAKER_NO, AVG(C.CAR_PRICE) AS CAR_AVG_PRICE,
COUNT(*) AS CAR_MAKER_COUNT
FROM CAR C GROUP BY C.CAR_MAKER_NO) CS;


--최대 판매가를 가진 차량의 정보를 조회
--car_no, car_name, car_maker_name, car_price
SELECT C.CAR_NO, C.CAR_NAME, CM.CAR_MAKER_NAME, C.CAR_PRICE 
FROM CAR C, CAR_MAKER CM
WHERE C.CAR_MAKER_NO = CM.CAR_MAKER_NO 
AND C.CAR_PRICE = (SELECT MAX(C.CAR_PRICE) FROM CAR C);
...
AND 
(
C.CAR_PRICE = (SELECT MAX(C.CAR_PRICE) FROM CAR C)
OR 
C.CAR_PRICE = (SELECT MIN(C.CAR_PRICE) FROM CAR C)
);

--차량 판매 정보 중 평균 판매대수 이상인 차량 정보를 출력
--car_no, car_name, car_maker_name, sale_count(판매대수)
--1. 차량별 총 판매대수 조회
SELECT CAR_NO, SUM(CAR_SELL_EA) AS SUM_SELL_EA FROM CAR_SELL
GROUP BY CAR_NO; 
--2. 1번 데이터를 전체 평균 판매대수 조회
SELECT TRUNC(AVG(SUM_SELL_EA),0) AS AVG_SELL_EA
FROM
(SELECT CAR_NO, SUM(CAR_SELL_EA) AS SUM_SELL_EA FROM CAR_SELL
GROUP BY CAR_NO)
--3. 1번과 2번 데이터를 비교해서 평균 이상인 데이터를 조회
SELECT C.CAR_NO, C.CAR_NAME, CM.CAR_MAKER_NAME, SUM(CS.CAR_SELL_EA)
FROM CAR C, CAR_MAKER CM, CAR_SELL CS
WHERE C.CAR_MAKER_NO = CM.CAR_MAKER_NO AND C.CAR_NO = CS.CAR_NO
GROUP BY C.CAR_NO, C.CAR_NAME, CM.CAR_MAKER_NAME
HAVING SUM(CS.CAR_SELL_EA) >= (SELECT TRUNC(AVG(SUM_SELL_EA),0) AS AVG_SELL_EA
FROM
(SELECT CAR_NO, SUM(CAR_SELL_EA) AS SUM_SELL_EA FROM CAR_SELL
GROUP BY CAR_NO));
